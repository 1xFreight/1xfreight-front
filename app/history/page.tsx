...
export default function History() {
  ...
  const getQuotesDebounced = useDebouncedCallback(() => {
    getWithAuth(
      `/quote/carrier/history?skip=${(page - 1) * paginationConfig.pageLimit}&limit=${paginationConfig.pageLimit}&searchText=${search ?? ""}&sort=${filters?.sort ?? ""}`,
    ).then((data) => {
      setQuotes(data);
      setLoading(false);
    });
  ...
  return (
    <div className={"history-quotes-page page"}>
